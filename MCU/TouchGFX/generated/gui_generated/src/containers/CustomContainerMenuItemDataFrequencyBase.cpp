/*********************************************************************************/
/********** THIS FILE IS GENERATED BY TOUCHGFX DESIGNER, DO NOT MODIFY ***********/
/*********************************************************************************/
#include <gui_generated/containers/CustomContainerMenuItemDataFrequencyBase.hpp>
#include <touchgfx/Color.hpp>
#include <texts/TextKeysAndLanguages.hpp>

CustomContainerMenuItemDataFrequencyBase::CustomContainerMenuItemDataFrequencyBase() :
    flexButtonCallback(this, &CustomContainerMenuItemDataFrequencyBase::flexButtonCallbackHandler)
{
    setWidth(480);
    setHeight(52);
    TextAreaBackground.setPosition(321, 2, 155, 50);
    TextAreaBackground.setColor(touchgfx::Color::getColorFromRGB(255, 255, 255));
    TextAreaBackground.setAlpha(0);
    add(TextAreaBackground);

    textArea.setPosition(324, 2, 148, 50);
    textArea.setColor(touchgfx::Color::getColorFromRGB(0, 0, 0));
    textArea.setLinespacing(0);
    Unicode::snprintf(textAreaBuffer, TEXTAREA_SIZE, "%s", touchgfx::TypedText(T___SINGLEUSE_2LYF).getText());
    textArea.setWildcard(textAreaBuffer);
    textArea.setTypedText(touchgfx::TypedText(T___SINGLEUSE_Z8R9));
    add(textArea);

    flexButton.setText(TypedText(T___SINGLEUSE_WP1Q));
    flexButton.setTextPosition(0, 0, 311, 50);
    flexButton.setTextColors(touchgfx::Color::getColorFromRGB(10, 10, 10), touchgfx::Color::getColorFromRGB(10, 10, 10));
    flexButton.setAction(flexButtonCallback);
    flexButton.setPosition(0, 2, 311, 50);
    add(flexButton);
}

CustomContainerMenuItemDataFrequencyBase::~CustomContainerMenuItemDataFrequencyBase()
{

}

void CustomContainerMenuItemDataFrequencyBase::initialize()
{

}

void CustomContainerMenuItemDataFrequencyBase::flexButtonCallbackHandler(const touchgfx::AbstractButtonContainer& src)
{
    if (&src == &flexButton)
    {
        //FlexButtonClicked
        //When flexButton clicked call virtual function
        //Call FlexButtonClicked
        FlexButtonClicked();
        //FadeTextAreaBackground
        //When flexButton clicked fade TextAreaBackground
        //Set alpha to 100 on TextAreaBackground
        TextAreaBackground.setAlpha(100);
        TextAreaBackground.invalidate();
    }
}
